...
MSG_RESYNC_REQ    declare integer
10 varset MSG_RESYNC_REQ

MSG_RESYNC_RESP   declare integer
11 varset MSG_RESYNC_RESP
...
$msg_type == $MSG_RESYNC_REQ        if handle_resync_req
$msg_type == $MSG_RESYNC_RESP       if handle_resync_resp
...
; ---------- RESYNCHRONIZE.REQ: запрос ресинхронизации ----------
handle_resync_req:
    ; PDU формата: [MSG_RESYNC_REQ]
    ; (дополнительных полей пока нет, можно расширить при необходимости)
    out CurrentSystemName() ": T_DATA.IND [INFO]: RESYNC_REQ PDU received – notify user"

    ; Уведомляем уровень представления о запросе ресинхронизации
    S_RESYNCHRONIZE.IND generateup

    return

; ---------- RESYNCHRONIZE.RESP: ответ на запрос ресинхронизации ----------
handle_resync_resp:
    ; PDU формата: [MSG_RESYNC_RESP]
    out CurrentSystemName() ": T_DATA.IND [INFO]: RESYNC_RESP PDU received – notify user"

    ; Уведомляем уровень представления, что ресинхронизация завершена
    S_RESYNCHRONIZE.CONF generateup
...
; Формируем PDU:
; [MSG_RESYNC_REQ]
connection_params bufferit 1 $MSG_RESYNC_REQ 1

out CurrentSystemName() ": S_RESYNCHRONIZE.REQ [INFO]: sending RESYNC_REQ via T_DATA.REQ"
T_DATA.REQ eventdown userdata $connection_params
...
; Формируем PDU:
; [MSG_RESYNC_RESP]
connection_params bufferit 1 $MSG_RESYNC_RESP 1

out CurrentSystemName() ": S_RESYNCHRONIZE.RESP [INFO]: sending RESYNC_RESP via T_DATA.REQ"
T_DATA.REQ eventdown userdata $connection_params
